...
export function createControlPlaneRepository(store: OperationStore = createMemoryOperationStore()) {
  ...
  snapshot(state: DemoPlatformState): ControlPlaneSnapshot {
    ...
    execute(command: ControlPlaneCommand, context: CommandContext = {}, now = new Date().toISOString()): OperationResult {
      ...
      if (command.kind === "connect-source" && command.authorizationMode !== "moyusi" && context.desktopConnected === false) {
        return store.put(completeOperation(pending, "failed", { error: commandError("desktop_offline", "Desktop 未连接，凭证没有离开本机", true) }, now));
      }
      ...
      if (command.kind === "create-deployment-order" && (!Number.isFinite(command.budgetLimitCny) || command.budgetLimitCny <= 0)) {
        return store.put(completeOperation(pending, "failed", { error: commandError("budget_required", "请先设置有效的费用上限", false) }, now));
      }
...
function commandResult(command: ControlPlaneCommand): Record<string, unknown> {
  ...
  if (command.kind === "create-deployment-order") return { deploymentId: "deployment_demo_next", modelId: command.modelId, status: "provisioning", budgetLimitCny: command.budgetLimitCny };
