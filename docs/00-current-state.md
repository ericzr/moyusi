...
当前目录已进入 Stage 2 本地 mock 纵向闭环，并完成工作台第一轮信息架构扩展：双产品面使用正式 URL，模型与供给选择能深链到对应工作台步骤；统一余额和共享算力来源可以一键切换、恢复上一配置、完成模拟调用，并把实际模型来源写入费用记录。工作台已按“开始 / 接入 / 工作环境 / 费用 / 设置”分组，新增工具与设备、会话、用量与请求入口。它仍不是生产系统；下一步是 Provider/Route 领域契约、BYOK ImportGrant、桌面端配置 diff 和有类型的 mock API 失败状态。
...
| 测试覆盖仍是基线 | 已有类型检查、13 个领域/契约用例和生产构建 | 尚未覆盖浏览器交互、可访问性与视觉回归 | 加 Catalog mock 合同测试与最小 E2E |
