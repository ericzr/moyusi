...
当前目录已进入 Stage 2 本地 mock 纵向闭环，并完成工作台第一轮信息架构扩展：双产品面使用正式 URL，模型与供给选择能深链到对应工作台步骤；统一余额和共享算力可以保存路由，BYOK 会经过 Desktop 本机授权，自有端点会先做地址与协议探测，专属算力会在确认预算后创建部署订单。活动路由可以恢复、完成模拟调用，并把实际模型来源写入费用记录。它仍不是生产系统；下一步是目录发布版本、网关计量事件链和桌面端配置 diff / 回滚。

## 当前可验证的产品资产

- 两个一级产品面：模型广场、工作台。
- 策展模型目录：先按语言、图片、视频选择任务，再筛选闭源 API、开放权重、共享/专属算力和自有端点，并比较多供给来源后带入工作台。
- 工作台子域：总览、路由、来源与凭证、模型与部署、工具与设备、配置与迁移、会话、用量与请求、余额与账单、账户与偏好。
- 五类供给的任务分流：直接路由、本机凭证、端点探测、共享算力和带预算上限的专属部署。
...
| 问题 | 当前证据 | 风险 | 下一步 |
| --- | --- | --- | --- |
| 路由与领域仍需继续拆分 | 已有 `/market/:modelId` 与 `/workspace/:section`，工作台已按任务分组但内容仍集中在单个 feature | 子域独立加载和权限边界尚未形成 | 按 routing、sources、deployments、tools、profile、sessions、usage、billing 拆分 |
| 状态仅为客户端演示 | 路由和部分配置保存在本地，operation 与部署队列刷新后重置 | 无服务端并发、持久化和审计 | 接持久化 control plane 与 operation 查询 |
| 数据仍为 fixture | 组件已只通过 catalogRepository 读取，但源数据仍在旧 fixture 文件 | 还没有加载、失败、过期和权限状态 | 将 fixture 移入独立目录，并由有类型 mock API 驱动 |
| 没有后端与桌面端 | 当前只有 Vite Web 原型 | 无法实现安全 Key、真实配置写入和调用 | 建 control plane、gateway 与 desktop 工程 |
| 测试覆盖仍是基线 | 已有类型检查、11 个测试文件、50 个领域/契约用例和生产构建 | 尚未覆盖浏览器交互、可访问性与视觉回归 | 加最小关键路径 E2E |
