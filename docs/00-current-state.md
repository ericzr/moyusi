...
当前目录已进入 Stage 2 本地 mock 纵向闭环：双产品面使用正式 URL，模型与供给选择能深链到对应工作台步骤；统一余额和共享算力来源可以一键切换、恢复上一配置、完成模拟调用，并把实际模型来源写入费用记录。它仍不是生产系统；下一步是 BYOK ImportGrant、桌面端配置 diff 和有类型的 mock API 失败状态。
...
| 问题 | 当前证据 | 风险 | 下一步 |
| --- | --- | --- | --- |
| 路由与领域仍需继续拆分 | 已有 `/market/:modelId` 与 `/workspace/:section`，但工作台内容仍集中在单个 feature | 子域独立加载和权限边界尚未形成 | 按 routing、sources、deployments、profile、billing 拆分 |
