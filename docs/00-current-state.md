...
当前目录是一个已完成双产品面收敛的交互前端预览，表达“闭源/开放模型选择 + 路由与部署 + 可迁移工作环境 + 用量账户”的产品架构，但仍不是生产工程。旧视觉导出、气候数据面板、占位品牌、仿真密钥和无关 UI 组件已迁出工作区；模型广场与工作台已经拆成 feature，下一步应接入 Router、有类型的 mock API 和自动化测试。

## 当前可验证的产品资产

- 两个一级产品面：模型广场、工作台。
- 策展模型目录：闭源 API、开放权重、共享算力和自有端点可搜索筛选并带入工作台。
...
| 非 Git 工作区 | 当前目录未检测到 `.git` | 无法审查历史或安全回滚 | 开发前初始化仓库或迁入受版本控制项目 |
